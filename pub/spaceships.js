...
function create() {
	...
	for (let i = 0; i < MAX_ENEMIES; i++){
		let randomH = Math.random() * scene_h;
		let randomW = Math.random() * (scene_w*10) + scene_w/2;
		enemies.push(this.add.image(randomW, randomH, "enemy"));
	}

	for (let i = 0; i < MAX_BULLETS; i++){
		bullets.push(this.add.image(-100, -100, "bullet"));
